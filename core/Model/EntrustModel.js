    ...
    async processOrder(reqItem, resItem, trigger_type_id) {
        ...
        try {
            ...
            let reqUpdExchangeCoinAssets = await cnt.execQuery(`update m_user_assets set available = available + ? , frozen = frozen - ? , balance = balance - ?
                                                            where user_id = ? and coin_id = ? and frozen >= ? and balance >= ?  `, [reqAvailableCoin, reqFrozenCoin, tradeAmount, reqItem.user_id, coinEx.exchange_coin_id,reqFrozenCoin, tradeAmount]);
            ...
            let resUpdCoinAssets = await cnt.execQuery(`update m_user_assets set frozen = frozen - ? , balance = balance - ?
                                                    where user_id = ? and coin_id = ?  and frozen >= ? and balance >= ? `, [tradeVolume, tradeVolume, resItem.user_id, coinEx.coin_id,tradeVolume, tradeVolume]);
